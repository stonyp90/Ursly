...
import { IAuthorizationService, AUTHORIZATION_SERVICE } from '../ports/entitlement.port';
 ...
 */
export const SkipEntitlementCheck = () => SetMetadata(SKIP_ENTITLEMENT_KEY, true);
...
export class EntitlementGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const skipCheck = this.reflector.getAllAndOverride<boolean>(SKIP_ENTITLEMENT_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (!permissionsMetadata) {
      ...
      if (!isValid) {
        throw new ForbiddenException('Your access has been suspended or is invalid');
      }
   ...
   */
  private extractUserFromRequest(request: Request): { id: string; email: string; organizationId: string } | null {
 ...
 */
export function hasAllPermissions(request: Request, permissions: string[]): boolean {
 ...
 */
export function hasAnyPermission(request: Request, permissions: string[]): boolean {
  const userPermissions = request.entitlementUser?.permissions ?? [];
